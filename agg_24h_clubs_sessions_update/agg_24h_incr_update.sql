-- Удаляем данные только за обрабатываемый час
...
-- Вставляем новые данные за период с буфером ±2 часа
...
WITH
    ...
    base_events AS (
        ...
        LEFT JOIN ddxfitness_prod_v2.pg_users u ON u.id = vh.user_id
    ...
    sessions_with_hours AS (
        SELECT
            ...
            arrayJoin(
              arrayMap(
                x -> toStartOfHour(
                       toDateTime('{date_from}', 'Europe/Moscow')
                       - INTERVAL 2 HOUR
                       + x * 3600
                     ),
                range(
