...
WITH
    dedup_users AS (
        SELECT
            id,
            argMax(phone, _peerdb_version) AS phone,
            argMax(last_name, _peerdb_version) AS last_name,
            argMax(name, _peerdb_version) AS name
        FROM ddxfitness_prod_v2.pg_users
        GROUP BY id
    ),
    dedup_group_training_time_tables AS (
        SELECT
            id,
            argMax(group_training_id, _peerdb_version) AS group_training_id,
            argMax(club_id, _peerdb_version) AS club_id,
            argMax(start_time, _peerdb_version) AS start_time,
            argMax(end_time, _peerdb_version) AS end_time,
            argMax(is_deleted, _peerdb_version) AS is_deleted
        FROM ddxfitness_prod_v2.pg_group_training_time_tables
        GROUP BY id
    ),
    cities_list AS (
        SELECT id,
    ...
    prepared_data AS (
        ...
        FROM dedup_group_training_time_tables AS gttt
        ...
        INNER JOIN dedup_users AS ur ON ur.id = emp.user_id
