...
with DAG(
    ...
    def check_anomalies(**kwargs):
        ...
        daily_inflow AS (
            ...
            FROM dedup_inflow_and_outflow_new
            WHERE is_inflow = 1
        ...
        WITH dedup_inflow_and_outflow_new AS (
            SELECT
                ...
                argMax(is_outflow, _peerdb_version) AS is_outflow
            FROM ddxfitness_prod_v2.inflow_and_outflow_new
            GROUP BY user_payment_plan_id
        ),
        daily_outflow AS (
