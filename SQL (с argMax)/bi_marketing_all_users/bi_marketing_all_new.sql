...
WITH
    dedup_user_payment_plans AS (
        SELECT
            id,
            argMax(discount_code_id, _peerdb_version) AS discount_code_id,
            ...
            argMax(start_date, _peerdb_version) AS start_date,
            ...
            argMax(card_token_id, _peerdb_version) AS card_token_id,
    ...
    dedup_users AS (
        SELECT
            ...
            argMax(sport_experience, _peerdb_version) AS sport_experience,
...
LEFT JOIN dedup_users ur ON f.id = ur.id
LEFT JOIN ddxfitness_prod_v2.pg_employees em ON f.employee_id = em.id
LEFT JOIN dedup_users ur2 ON ur2.id = em.user_id
