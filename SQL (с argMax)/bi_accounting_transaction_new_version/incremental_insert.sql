...
WITH
    ...
    dedup_transactions AS (
        SELECT
            id,
            argMax(receipt_id, _peerdb_version) AS receipt_id,
            argMax(club_legal_info_id, _peerdb_version) AS club_legal_info_id,
            argMax(parent_id, _peerdb_version) AS parent_id
...
LEFT JOIN dedup_users AS u ON actr.user_id = u.id
LEFT JOIN (
    SELECT *
